...
def back_test(y_preds,y_true):
    ...
    while i > -len(y_true)+1:
        ...
        i -= 1
    return print('Predicted correct:', same_sign_list.count('1'),'Predicted wrong:',same_sign_list.count('0'),'Percent_total_true:', sum(true_list),'Percent_total_Preds:',sum(prediction_list),'Prediction for today:',(y_preds[-2] - y_preds[-3])/y_preds[-2] *100,'Prediction for tommorow:',(y_preds[-1] - y_preds[-2])/y_preds[-1] *100)
