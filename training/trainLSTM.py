...
def train_model(ensembly = False):
    x_t, y_t, x_val, y_val, x_test_t, y_test_t,size,money_train,money_val,money_test = data_prep('pancake',ta=True,initial_training=True,batch=False,SS_path = SS_path,MM_path = MM_path,big_update=False)
    ...
    history_lstm = lstm_model.fit(x=[trim_dataset(x_t, BATCH_SIZE),trim_dataset(y_t, BATCH_SIZE)],y=trim_dataset(y_t, BATCH_SIZE), epochs=10000,
                                  ...
                                  shuffle=False, validation_data=([trim_dataset(x_val, BATCH_SIZE),trim_dataset(money_val,BATCH_SIZE),trim_dataset(y_val,BATCH_SIZE)],
